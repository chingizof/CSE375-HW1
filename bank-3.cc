// make our hashmap more thread friendly. Divide map into 10 segments, where each segment will have own lock.
...
const float initial_amount = 100.0f;
...
int main() {
  std::vector<int> idx_threads = {1, 2, 4, 8, 16, 25};
  ...
  for (int num_threads: idx_threads) {
    ...
    for (int i=0; i < num_accounts; i++) {
      bank.insert({i, 100}); //insert by 10$
    }
