...
float balance() {
  ...
  for (int i=0; i < num_accounts; i++) {
    std::lock_guard<std::mutex> lock(mutexes[i % num_segments]);
    total += bank[i];
  }
...
double dowork(int local_dowork) {
...
int main() {
  ...
  for (int num_threads: idx_threads) {
    ...
    for (int i=0; i < num_threads; i++) {
      futures.emplace_back(std::async(std::launch::async, dowork, (int) num_dowork/num_threads));
    }
