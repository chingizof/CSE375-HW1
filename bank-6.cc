...
//   }
  ...
  // how to make it faster, i need to lock the whole global mutex on a map for this, how to handle it in deposit()?
  global_mutex.lock();
  ...
  for (int i = 0; i < num_segments; i++) {
    for (int j = i; j < num_accounts; j += num_segments) {
        total += bank[i];
    }
    mutexes[i].unlock();
  }
