...
void deposit(std::mt19937 &generator) {
  ...
  int segment1 = b1 % num_segments, segment2 = b2 % num_segments;
  ...
  int first_seg = std::min(segment1, segment2);
  int second_seg = std::max(segment1, segment2);

  std::lock_guard<std::mutex> lock1(mtx[first_acc]);
  std::lock_guard<std::mutex> lock2(mtx[second_acc]);
  if (segment1 != segment2) {
    std::lock_guard<std::mutex> lock3(blnc_mtx[first_seg]);
    std::lock_guard<std::mutex> lock4(blnc_mtx[second_seg]);
    global_balances[segment1] += v;
    global_balances[segment2] -= v;
  }
  bank[b1] -= v;
  bank[b2] += v;
}
